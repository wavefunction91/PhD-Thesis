...
\subsection{Formal and Mean Field Solutions to the Wave Equation for Time Independent 
...
known as the Fock operator corresponding to \cref{eq:GenElHam} \todo{cite}. The non--linearity of \cref{eq:HFNonCan} comes in the fact that the operators
...
Further note that $\{\tilde \phi_i\}$ and $\{\phi_i\}$ generate the same Slater determinant \co{make a note about this in previous section}.
...
phenomena. There is another, more subtle problem relating to the form \label{eq:HKFunc} in that the exact form for
