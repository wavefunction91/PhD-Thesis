...
\begin{equation}
  \spop{\vc{\rho}}(\vc{r}) = \sum_K \spop \rho^K(\vc{x}) \pauli{K},
\end{equation}
where
\begin{equation}
  \spop \rho^K(\vc{x}) = \sum_i \delta^3(\vc{r} - \vc{r}_i) \otimes \pauli{K}.
\end{equation}
For a particular Slater determinant, \cref{eq:SlaterCondon,eq:DiracInts} yield
\begin{equation}
  \vc{\rho}(\vc{r}) = \sum_K \innerop{\Phi^N_I}{\spop{\rho}^K(\vc{x})}{\Phi^N_I}\pauli{K} = 
    \sum_K \sum_{i \in \hilb{K}_I^N} \rho_{ii}^K(\vc{r}) \pauli{K}
\end{equation}
...
In the spinor representation, these operators take the general form\todo{fix typesettting here}
\begin{align}
  \label{eq:SpinorKinMom}
  &\spop{\vc{p}}(\vc{x}_\xi) = -\ii \nabla_\xi \otimes \op{I}_S(\xi) \quad \Longrightarrow \quad 
  \spop{T}^\NR(\vc{x}_\xi) = - \frac{1}{2} \Delta_\xi \otimes \op{I}_S(\xi),
\end{align}
\begin{align}
...
the quantum treatment of the electrons and the nuclei such that they may be treated separately. To this end,
...
\begin{equation}
  \ket{\Psi_{tot}} = \ket{\Psi_{el}} \otimes \ket{\Theta_{nuc}}.
\end{equation}

\todo{Talk about BO approximation}

\begin{equation}
  \label{eq:BOH}
  \op{H}^\NR_{el} - E_{nn} \approx \sum_i \op{T}^\NR(i) + \op{V}_{ne}(i) + \sum_{i\neq j} \op{g}^C(i,j)
\end{equation}
\begin{align}
