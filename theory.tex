...
In atomic units, the DCB Hamiltoninan for $N$ electrons is given by \todo{cite}
\begin{equation}
...
\begin{alignat}{2}
&\op{h}^D(i) = 
c \vc{a}(i) \cdot \op{\vc {p}}(i) + (\vc{b} - \vc{I}_4) c^2, \qquad &&\text{Dirac Hamiltonian},\\
%
...
\end{align}
where $\vc{I}_2$ and $\vc{I}_4$ are the 2-by-2 and 4-by-4 identity matrices, respectively, and have used the shorthand notations
\begin{align}
  \vc{a} \cdot \op{\vc{p}}  &= \left( \op{p}_x \otimes \pauli{1} + \op{p}_y \otimes \pauli{2} + \op{p}_z \otimes \pauli{3} 
