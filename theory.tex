...
for a particular Slater determininant we obtain,
\begin{equation}
...
multiplicitive. In such cases, expectation values in the $I$-th Slater determinant admit a simple form,
\begin{equation}
...
repsectively. $L_\xi$ is the charge of the $\xi$-th particle, which in atomic units is given by $-1$ for electons and 
$Z_A$ for the $A$--th nucleus, where $Z_A$ is the number of protons in said nucleaus. 
...
Born--Oppenheimer ansatz for the molecular wave function) \todo{cite}. This is typically a safe assumption for reasonably
...
the nuclear configuration is approximately static, and the electons only ,,feel" the electrostatic
...
\end{align}
$\rho^0_{nuc,A}(\vc{R})$ is the scalar density of the $A$--the nucleaus defined through a proper generalization of
...
\end{equation}
This critera is clearly met if $\rho_{nuc,A}(\vc{R})$ is a normalized function. In this work, we will approximate
...
\end{equation}
and will be referred to as the electronic Schr\"{o}dinger equation within the Born--Oppenheimer approximation \todo{cite}.
In this work, all non--relativistic specializations of \cref{eq:QWave} will referred to as some variant of the Schr\"{o}dinger
equation due to the explcit presence of the kinetic energy operator in the Hamiltonian. This is due to the fact that they
are specialization of the original Schr\"{o}dinger Hamiltoninan of the form
\begin{equation}
\label{eq:SchHam}
...
and perhaps the most glaring in the context of molecular calculations envolving the quantum treatment of electrons,
is that in the limit of electrostatics in the absence of external fields there is only trivial action of the
Hamiltonian onto the spin components of the electronic  wave function. To be clear, electronic spin is inherient in any quantum
treatment of electrons as it manifests naturally through the irredicuble representations of the Galiei group
which governs non--relativistic mechanics \todo{cite}. Phenomenologically, one may introduce non--trival spin manipulation into the non--relativistic
Hamiltoninan through the interaction with an external magnetic field, $\vc{B}$, via the spin--Zeeman term,
\begin{equation}
...
experiment\todo{cite Sakurai}. However, it is well known that $\op{H}^\mathrm{Zeeman}$ only yields a proper physical description
of the electronic spin degrees of freedom in the strong field limit \todo{cite}, and clearly yields no mechanism to treat
intrinsics effects such as zero--field splitting of electronic energy levels.
...
incapible of adhering to the laws of special relatively, i.e. it is impossible to write down a Hamiltoninan of the form in
\cref{eq:SchHam} which is Lorentz covariant. This problem is easily identified through recognizing that the Schr\"{o}dinger
equation is quadratic in spatial coordinates through $\op{T}^\NR$ and linear in time, and thus incabible of being compatible with Lorentz boosts. At first glance,
...
The realization of relativstic effects in light elements typically manifests in context of the \emph{ab initio} introduction of spin couplings
into the Hamiltoninan, which not surptisingly also solves the aformentioned problem with treating electronic spin non--relativistically. 
The following section provides a brief overview of the treatement of relativistic effects in molecular quantum mechanics.
