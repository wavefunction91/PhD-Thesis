...
\begin{align}
  &\vc F^{\HF,0} = \vc H^\mathrm{core,0} + \vc J^0 - \vc K^0, \\
  &\vc F^{\HF,k} = - \vc K^k, \quad k \in \{1,2,3\},
\end{align}
and KS Fock matrix (\cref{eq:KSMat}),
\begin{align}
  &\vc F^{\KS,0} = \vc H^\mathrm{core,0} + \vc J^0  +\vc V^{xc,0}, \\
  &\vc F^{\KS,k} = \vc V^{xc,k}, \quad k \in \{1,2,3\},
\end{align}
where 
\begin{subequations}
...
manipulating effective two--component relativistic hamiltonians in this work, the explicit development of
...
however it will drastically simplify the development of practical two--component relativistic methods
in finite basis sets.


\subsection{Two--Component Relativistic Mean Field Wave Functions}
\label{sec:MF2C}
...
way as to effectively include these many--body effects by way of the B\"{o}ttiger \co{ check spelling} scaling,
\begin{equation}
\vc h^\XtC \mapsto \vc \Lambda^{B\dagger} \vc h^\XtC \vc \Lambda^{B},
\end{equation}
where $\vc \Lambda^B$ is the sparse matrix of B\"{o}ttiget scaling factors \todo{cite}. 
This scaling method
