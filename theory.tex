...
\section{Formal Solutions of the Wave Equation for Time Independent Electronic Hamiltonians}
\label{sec:MF}
...
\end{equation}
where $\spvc F^\HF, \spvc S \in \MatSpace{C}{2N_b}{2N_b}$ and are given by
\begin{align}
&\spvc{F}^\HF = \begin{bmatrix} \vc{F}^{\HF,\saa} & \vc{F}^{\HF,\sab} \\ \vc{F}^{\HF,\sba} & \vc{F}^{\HF,\sbb} \end{bmatrix}, \\
%&\spvc{C}^\HF = \begin{bmatrix} \vc{C}^{\HF,\alpha} \\ \vc{C}^{\HF,\beta} \end{bmatrix},\\
...
%&\spn{\epsilon}^\HF_{pq} = \delta_{pq} \epsilon^\HF_p.
\end{align}
\end{subequations}
Here, the rectangular matrices $\vc{C}_o^{\HF,\sigma} \in \MatSpace{C}{N_b}{N}$ are the spin--basis coefficients which expand the
occupied HF-MOs (in the sense of second quantization) in the basis set. $\spvc \epsilon_o^\HF \in \MatSpace{R}{2N_b}{N}$ is the diagonal matrix of 
occupied HF-MO eigenenergies. 
...
\end{equation}
where $\spvc{C} \in \MatSpace{C}{2N_b}{2N_b}, \spvc \epsilon^\HF \in \MatSpace{R}{2N_b}{2N_b}$ represent a full rank eigendecomposition
of $\spvc F^\HF$. Thus from a set of $N_b$ basis functions, one may construct a full--rank ...

\todo{2N spinors from N basis}
\todo{Span of basis is span of C}
\todo{Iterative (brief)}

\section{Density Functional Theory}
\label{sec:DFT}

The Hartree--Fock approximation provides an excellect reference determinant for systematic improvement
...
description of the many--body wave function for all but he simplest problems and any reasonable improvement
of the wave function leads to computationally intractible problems for experimentally relavent molecular 
systems. Luckily, the deficencies of the Hartree--Fock approximation are well known: it simply lacks
explicit two--body interactions in the effective Hamiltonian, i.e. the non--interacting system is said to
lack electron correlation. Thus if one were able to account for electronc correlation 
 in an effective one--body Hamiltonian, one would not need to use expansive many--body expansions
...
The original Hohenberg--Kohn (HK) theorem states that there exists a bijection between an external scalar potential and
the scalar one--body density of the many--body wave function \todo{cite Hohenberg}. More generally, the HK theorem may be extended to 
the 1RDM if the external potential has spin structure or is generally non--local in character \todo{cite Gilbert}. Thus the total energy of the many--body state may
be written in terms of functionals of the 1RDM,
\begin{equation}
  \label{eq:HK}
...
all one and two body interactions separate from the external pontential exactly. $F^\mathrm{HK}$ may be
...
\end{equation}
where $H^\mathrm{free}$ is the energy functional pertaining to the free particle part of the electronic Hamiltoninan
describing the system, $J$ is the classical Coulomb functional of \cref{eq:ClassCoulFunc}, and $E^{xc}$ is the
\emph{exchange--correlation} (xc) functional which describes all purely quantum many body energetic contrubtions, including those
which enforce the proper spin statistics (i.e. Fermi--Dirac in the case of fermions). Thus given the exact forms 
of $H^\mathrm{free}$ and $E^{xc}$, one all information pertaining to the physical system.

However, there are a number of problems implicit in \cref{eq:HK,eq:HKFunc}. The most problematic is of course that the 
exact form of $E^{xc}$ is unknown, thus rendering the practical treatment of DFT as an exact method impossible. In
practice however, the lack of knowledge of an exact $E^{xc}$ is not so aggregious that it becomes an impractical
...
$H^\mathrm{free}$ \todo{solidify this arugement}.

\todo{confirm this section}
A solution to the problem of handling $H^\mathrm{free}$ in a consistent manner comes from Kohn--Sham density functional
theory (KS-DFT), where one reintroduces the concept of a fictitious non--interacting (Slater determinant) 
wave function to represent the 
true many--body wave function \todo{cite KS}. As such the total energy of the system may be written as,
\begin{equation}
  \label{eq:KSFunc}
  E^\mathrm{KS}[\gamma_s^1] = H_s^\mathrm{free}[\gamma_s^1] + J[\gamma_s^1] + E^{xc'}[\gamma_s^1] + 
    \iint_\mathbb{F} \op{v}(\vc{x};\vc{x}') \gamma_s^1(\vc{x}'; \vc{x})\, \dd^4\vc{x}\dd^4\vc{x}',
\end{equation}
where all of the moieties with an $s$ subscript denote that they relate to the fictitious non--interaction system,
i.e. a single Slater determinant such that $\gamma^1_s$ may be composed as \cref{eq:SpinorDensityMat}. $E^{xc'}$ is
an augmented xc functional which accounts for the difference of $H_s^\mathrm{free}$ and $H^\mathrm{free}$,
\begin{equation}
  E^{xc'}[\gamma^1_s] = H^\mathrm{free}[\gamma^1_s] - H_s^\mathrm{free}[\gamma^1_s] + E^{xc}[\gamma^1_s].
\end{equation}
Due to the introduction of the non--interacting system, or eqivilently a set of orbitals as in the Hartree--Fock approximation,
we may derive an analogous set of SCF equations \todo{define SCF in previous section} which minimize \cref{eq:KSFunc}.
In analogy of \cref{eq:HFEq} \todo{change this when updated to reflect ident of lambda with eps}, we may write
\begin{equation}
  \int_{\mathbb F} \spop{F}^\mathrm{KS}(\vc{x}_1; \vc{x}_1') \phi_i(\vc{x}_1') \,\dd^4\vc{x}'_1 = 
    \epsilon_i \phi_i(\vc{x}_1), 
\end{equation}
where
\begin{align}
  &\spop{F}^\mathrm{KS}(\vc{x}; \vc{x}') = 
    \spop{H}^\mathrm{core}(\vc{x}; \vc{x}') + \spop{J}(\vc{x}; \vc{x}') +
    \spop{V}^{xc}(\vc{x}; \vc{x}') \\
  &\spop{V}^{xc}(\vc{x}; \vc{x}') = \frac{\delta E^{xc'}[\gamma^1_s]}{\delta \gamma^1_s(\vc{x}';\vc{x})}
\end{align}
...
%For a particular Slater determinant, $\ket{\Phi_0^N}$, \cref{eq:EnergyFunctional} may be recast into
%a form which solely depends on the 1RDM due to \cref{eq:RDMSD},
%\begin{equation}
%E[\Phi_0^N] \equiv E[\gamma^1] = h[\gamma^1] + J[\gamma^1] + E^{xc}[\gamma^1] + V[\gamma^1]
%\end{equation}
...
the number of protons for a particular nucleaus, repsectively . 
...
potential of a fixed nuclear wave function at any given time. This approach is therefore analogus to
...
In the absense of external fields , the DCB Hamiltoninan for $N$ electrons is given in atomic units by \todo{cite}
\begin{equation}
  \label{eq:DCBHam}
...
\cref{eq:SpinorCoulomb}, repectively, and
\begin{alignat}{2}
&\op{h}^D(i) = 
...
\end{alignat}
Here we have denoted the the Dirac matricies as
\begin{align}
...
serveral aspects of \cref{eq:BreitEq} which will play an important role in the following developments. The block structure
...
crutial to a rigorus treatment of relativistic effects. 
