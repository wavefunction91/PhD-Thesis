...
\begin{equation}
\op{H} \ket{\Psi(t)} = \ii \partial_t \ket{\Psi(t)}, 
...
The nature of $\op H$ for various physical situations and approximations relavent
...
(or more specifically, Hartree products) will be a rank-$N$ tensor contraction over spin indicies as depicted in \cref{eq:SpinorOpInner1} 
...
$\vc{e}_\alpha$ and $\vc{e}_\beta$ to that of the Pauli matricies,
\begin{equation}
...
\end{equation}
An explicit derivation for the form of the general rank--$2N$ tranformation is given in \cref{apx:SpinorOp}. Here we state the special case of
one particle spinor operators as it will provide the basis for many of the manipulations in subsequent developments,
\begin{subequations}
\begin{align}
&\op{O}^{1,0}(\vc{r}) = \frac{1}{2} \left( \op{O}^{1,\alpha\alpha}(\vc{r}) + \op{O}^{1,\alpha\alpha}(\vc{r}) \right),\\
...
&\op{O}^{1,3}(\vc{r}) = \frac{1}{2} \left( \op{O}^{1,\alpha\alpha}(\vc{r}) - \op{O}^{1,\alpha\alpha}(\vc{r}) \right).
\end{align}
\end{subequations}
...
One may construct representations of Slater determinants (\cref{eq:SlaterDet}) is a consistent manner in this formalism through
...
respectively, and the notion of a zero particle occupation vector known as the vacuum, $\Vac \in \hilb{F}$, such that
\begin{subequations}
...
\end{equation}
where $\ket{\Phi^N_I}_\hilb{F} \in \hilb{F}$ is the occupation vecotor representation of $\ket{\Phi^N_I}$.
...
\end{equation}
where, in atomic units\co{Glossary}, \co{Consider reformatting these equations into columns to free up white space}
\begin{subequations}
\begin{align}
...
In the spinor representation of \cref{eq:SpinorOpPauli}, these operators are given in Cartesian coordinates by \todo{fix typesettting here},
\begin{align}
&\spop{\vc{p}}(\vc{x}_\xi) = \frac{\ii}{2} \nabla_{\vc{r}_\xi} \otimes \pauli{0}, \label{eq:MomentumSpinor}\\
&\spop{g}^C(\vc{x}_\xi,\vc{x}_\zeta) = \frac{1}{4} \frac{1}{\vert \vc{r}_\xi - \vc{r}_\zeta \vert } \otimes \pauli{0} \otimes \pauli{0},
  \label{eq:CoulombSpinor}
\end{align}
where $\xi$ and $\zeta$ can be either electronic or nuclear coordinates. 
\begin{align}
  \spop{T}_{el}(\vc{x}_1, \vc{x}_2,\ldots) = \frac{1}{4}\sum_i^{N_\mathrm{el}} \Delta_{\vc{x}_i} \otimes \pauli{0}
  \spop{T}_{nuc}(\vc{x}_1, \vc{x}_2,\ldots) = \frac{1}{4}\sum_i^{N_\mathrm{el}} \Delta_{\vc{x}_i} \otimes \pauli{0}
\end{align}
