...
effects such as spin--orbit coupling become energetically competitive.
...
In atomic units, the DCB Hamiltoninan for $N$ electrons is given by
\begin{equation}
\op{H}^\DCB = \sum_i^N  \op{h}^D(i) + \frac{1}{2} \sum_{i \neq j}\op{g}^C(i,j) \otimes \vc{I}_2 - \frac{1}{2} \sum_{i \neq j} \op{g}^B(i,j),
\end{equation}
where $\op{g}^C$ is the classical Coulomb interactions of \cref{eq:SpinorCoulomb} and
\begin{alignat}{2}
&\op{h}^D(i) = 
% \begin{bmatrix} 
%   \op{v}(i)  & c\vec{\pauli{}(i)} \cdot \op {\vc p(i)} \\
%   c \vec{\pauli{}(i)} \cdot \op{\vc{p}(i)} & \op{v}(i) - 2c^2\vc{I}_2
% \end{bmatrix}
\op{v}(i) \otimes \vc{I}_2 + c \vc{a}(i) \cdot \op{\vc {p}}(i) + (\vc{b} - \vc{I}_4) c^2, \qquad &&\text{Dirac Hamiltonian},\\
%
&\op{g}^B(i,j) = \frac{1}{2r_{ij}} \left( \vc{a}(i)\cdot\vc{a}(j) - \frac{(\vc{a}(i) \cdot \vc{r}_{ij}) (\vc{a}(j) \cdot \vc{r}_{ij})}{r_{ij}^2}\right), 
\qquad && \text{Breit interaction}.
\end{alignat}
