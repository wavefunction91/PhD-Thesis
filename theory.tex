...
ourselves to Hamiltoninans which are independent of time, though this will not be the case in general
(see \cref{sec:SCLMI} for instance).
Within this consideration, the electronic Hamiltonians one typically encounters for molecular systems with $N_\mathrm{el}$ electrons,
...
\end{equation}
where $t_0$ is a referece time point for which one may specify an initial condition for the electronic 
wave function. Without loss of generality, we will typically take $t_0 = 0$. We now consider the eigen
spectrum of the electronic Hamiltoninan such that there exists a set of wave functions $\ket{\Psi_n} \in \hilb{H}^N$
which yield
\begin{equation}
\label{eq:TIQWave}
...
in the following. As a spectral decomposition of $\op{H}_{el}$, $\hilb{H}^{N_\mathrm{el}} = \Span{\set{\ket{\Psi_n}}}$
such that any arbitrary vector in $\hilb{H}^{N_\mathrm{el}}$ may be decomposed as
\begin{equation}
\ket{\Psi (t) } = \sum_n c_n(t) \ket{\Psi_n},\qquad c_n(t) = \inner{\Psi_n}{\Psi(t)}.
\end{equation}
Here, the coefficients are assumed to be unit normal, i.e. $\sum_n\vert c_n (t) \vert = 1$. 
...
\end{equation}
Time dependent solutions such as comprise a special class of solutions to \cref{eq:QWave} 
known as stationary states. As $E_m$ is a real constant (due to the hermiticity of $\op{H}_\mathrm{el}$),
\cref{eq:StatSol} is referred to as stationary as it trivially rotates through the complex plane
such that for operators void of differentations with respect to time, $\op{O}^N$
\begin{equation}
\innerop{\Psi(t)}{\op{O}^N}{\Psi(t)} = \innerop{\Psi_m}{\op{O}^N}{\Psi_m} \quad \forall t,
\end{equation}
i.e. the state itself is not, strictly speaking time independnet, but its expectation values are.
...
for  true many body wave functions.
...
\begin{equation}
\label{eq:FockOp}
\op{f}^N = \sum_i^{N_\mathrm{el}} \op{f}^1(i), \qquad \op{f}^1(i) = \op{h}(i) + \op{v}^\mathrm{eff}(i),
\end{equation}
admit eigen functions which may be written as single Slater determinants. That is to say
that given a set $\mathcal{C}$ whos elements are eigen functions of $\op{f}^1(1)$,
\begin{equation}
\label{eq:FockEq}
\op{f}^1(1) \ket{\phi_p} = \ket{\phi_p} \epsilon_p,
\end{equation}
the Slater determinant which may be constructed from $\mathcal{C}$ are eigen functions of
...
in \cref{eq:FockOp}, the resulting solution constructed from \cref{eq:FockEq} will be referred
to as a \emph{non--interacting} system, i.e. they particles are treated as independent particles
in an external potential $\op{v}^\mathrm{eff}$. The energy of this non--interacting system,
\begin{equation}
\label{eq:NIE}
E_\mathrm{NI} = \sum_i^{N_\mathrm{el}}  \epsilon_i,
\end{equation}
is referred to as the non--interacting electronic energy of the system. If one wishes to minimize
$E_\mathrm{NI}$ over the possible Slater determinants which may be constructed from $\mathcal{C}$,
one must select the $N_\mathrm{el}$ orbtials with the lowest eigenenergy to build the determinants.
This choice yields an ordering on $\mathcal{C}$ which orders orbitals in increasing orbital eigenenergy.
This ordering also happens to correspond indentically to the phenomononlocal ordering dictated by the
