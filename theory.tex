...
in an external potential $\op{v}^\mathrm{eff}$. Given an configuration subset, $\mathcal{K}^{N}_I \subset \mathcal{C}$,
...
written as a functional of the 1RDM which may be constructed from that configuration \cite{Yang89_book}, \todo{make sure this is correct for 2C}
\begin{align}
&E^\mathrm{HF}[\mathcal{K}^N] = E[\gamma^1] = \iint_{\mathbb F} 
  \delta^4(\vc{x}_1 - \vc{x}'_1) \left[\left( \op{h}(\vc{x}_1) + \op{v}(\vc{x}_1) \right) \gamma^{1}(\vc{x}'_1; \vc{x}_1)\right] \,\dd^4\vc{x}_1 \dd^4\vc{x}'_1 \,\,+ \nonumber\\
   &\quad    \iint_{\mathbb{F}} \op{g}(\vc{x}_1,\vc{x}_2) 
     \left( \gamma^{1}(\vc{x}_1; \vc{x}_1)\gamma^{1}(\vc{x}_2; \vc{x}_2) - \gamma^{1}(\vc{x}_1; \vc{x}_2)\gamma^{1}(\vc{x}_2; \vc{x}_1) \right) \,\dd^4\vc{x}_1 \dd^4\vc{x}_2.
\end{align}
...
\begin{equation}
    ...
    \sum_{j,k \in \mathcal{K}^N} \lambda_{jk} \frac{\delta \inner{\phi_j}{\phi_k}}{\delta \phi_i(\vc{x})}  = 0 .
\end{equation}
...
conjugates of eachother, thus requiring both terms to go to zero simultaneously. Factoring out the spin integration and rearranging the summation indices, 
\cref{eq:RawHFEq} may be written as
\begin{align}
  \label{eq:HFNonCan}
  \sum_{j \in \mathcal{K}^N} \sum_{\sigma'} 
  \int_{\mathbb R^3} \left( \delta_{ij} F^{\sigma\sigma'}(\vc{r}; \vc{r'})  - \delta^3(\vc{r} - \vc{r}')\delta_{\sigma\sigma'}\lambda_{ij} \right)
    \phi_j^{\sigma'}(\vc{r}')\,\dd^3\vc{r}' = 0
\end{align}
where \todo{come back to this}
\begin{align}
&F^{\sigma\sigma'}(\vc{r}; \vc{r}') = 
  \frac{\delta E^\mathrm{HF}[\gamma^1]}{\delta \gamma^{1,\sigma'\sigma}(\vc{r}'; \vc{r})} = H^{\sigma\sigma'}(\vc{r};\vc{r}') + J^{\sigma\sigma'}(\vc{r};\vc{r}') - 
    K^{\sigma\sigma'}(\vc{r};\vc{r}') \\
&H^{\sigma\sigma'}(\vc{r}; \vc{r}') = \delta^3(\vc{r} - \vc{r}') \left(\op{h}^{\sigma\sigma'}(\vc{r}) + \op{v}^{\sigma\sigma'}(\vc{r}) \right) \\
&J^{\sigma\sigma'}(\vc{r}; \vc{r}') = \delta^3(\vc{r} - \vc{r}') 
\end{align}
\Cref{eq:HFNonCan} is the general form of a set of non--linear integrodifferential equations known as the Hartree--Fock equations and $\spop{F}$ is
known as the Fock operator corresponding to \cref{eq:GenElHam} \todo{cite}.
Using the fact that $\gamma^1$ is invariant to unitary transformation of the orbitals which comprise its construction \cite{Ostlund12_book,Yang89_book},
we may choose to transform \cref{eq:HFNonCan} in such a way as to diagonalize the matrix of lagrange multipliers, 
$\op{\mathcal U}: \lambda_{ij} \rightarrow \tilde\lambda_{i} \delta_{ij}$,
\begin{equation}
\label{eq:HFEq}
\int_{\mathbb F} \spop{F}(\vc{x}; \vc{x}') \tilde\phi_i(\vc{x}') = \tilde\lambda_i \tilde\phi_i(\vc{x}), \qquad \tilde\phi_i = \op{\mathcal U}\phi_i. 
\end{equation}
\Cref{eq:HFEq} is known as the canonnical Hartree--Fock equation and $\tilde \phi_i$ is referred to as a canonnical Hartree-Fock orbital. 
Equating \cref{eq:HFEq} with \cref{eq:FockEq}.
