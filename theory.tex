...
\end{align}
Here we have made an assumption which is crutial to the following developments, namely that the operators that we will work with
...
However, it will be the case that the linear operators which correspond to observables of the physical system will typically be spatically local,
...
The superscripted moeities $\op{O}^{N,\sigma_1\sigma'_1\cdots}$ will be refered to as the spinor operator
...
\end{align}
\end{subequations}
The coefficients labelled $\op{O}^{N,K_1K_2,\cdots}$ will be referred to as the spinor operator coefficients relative
to the Pauli matrices, or simply the Pauli coefficients for breviety.
This representation is especially convinient when the operator in question is hermetian, i.e. $\op{O}^N = \op{O}^{N\dagger}$.
In \cref{eq:SpinorOpN}, hermeticity of $\op{O}^N$ simply implies that the \emph{product} of the operator coefficients and 
basis vectors must be invarient under the hermetian adjoint and states nothing about the hermeticity of the coefficients
themselves. Due to the fact that the Pauli matricies are self adjoint, \cref{eq:SpinorOpPauli} implies that $\op{O}^N$
is hermetian if and only if the Pauli coefficients are hermetian. Working with hermetian operators
carries a number of benifits, most notably that the spectrum of such operators is strictly real.
...
\end{equation}
Clearly, $\op{\gamma}^N$ is an hermetian $N$--particle on $\hilb{H}^N$ which projects its elements onto $\ket{\Psi^N}$.
The density matrix plays a very central role in the development of quantum mecanical theory in that
...
\end{align}
From the $N$--particle density matrix, it is possible to define $P$--particle reduced density matricies ($P < N$)
...
\end{align}
which are hermetian projectors onto the $P$--particle subspaces of $\hilb{H}^N$ which construct $\ket{\Psi^N}$. 
Of particular interest to this work are the one-- and two--particle reduced density matricies,
\begin{align}
...
prove to be of great utility in the following developments.
...
the spin basis..
As was the case for spinor operators in \cref{sec:LO}, hermeticity of the density matrix does not
place any specific restrictions on the hermeticity on the spinor coefficients.
In analogy to \cref{eq:SpinorOpPauli}, we may construct hermetian coefficients via
\begin{equation}
  \spn{\gamma}^P(\vc{x}_1,\vc{x}_2,\ldots;\vc{x}'_1,\vc{x}'_2,\ldots ) \equiv 
...
\end{align}
\end{subequations}
where all of the Pauli components of the spinor operator and RDMs are hermetian. In the case of a single Slater 
...
\begin{align}
  &\gamma_I^0(\vc{r};\vc{r}') = \frac{1}{2}   \sum_{i\in\hilb{K}^N_I}\left(\phi^{\alpha}_i(\vc{r})\phi^{\alpha*}_i(\vc{r}') + \phi^{\beta}_i(\vc{r})\phi^{\beta *}_i(\vc{r}') \right), \\
  &\gamma_I^1(\vc{r};\vc{r}') = \frac{1}{2}   \sum_{i\in\hilb{K}^N_I}\left(\phi^{\alpha}_i(\vc{r})\phi^{\beta *}_i(\vc{r}') + \phi^{\beta}_i(\vc{r})\phi^{\alpha*}_i(\vc{r}') \right), \\
  &\gamma_I^2(\vc{r};\vc{r}') = \frac{\ii}{2} \sum_{i\in\hilb{K}^N_I}\left(\phi^{\alpha}_i(\vc{r})\phi^{\beta *}_i(\vc{r}') - \phi^{\beta}_i(\vc{r})\phi^{\alpha*}_i(\vc{r}') \right), \\
  &\gamma_I^3(\vc{r};\vc{r}') = \frac{1}{2}   \sum_{i\in\hilb{K}^N_I}\left(\phi^{\alpha}_i(\vc{r})\phi^{\alpha*}_i(\vc{r}') - \phi^{\beta}_i(\vc{r})\phi^{\beta *}_i(\vc{r}') \right). 
\end{align}
\end{subequations}
The exrpressions for the 2RDM may be derived from the Grassmann product form in \cref{eq:RDMSD}.


In the case where the operator in equation is scalar multiplicitive, i.e. does not contain differential operators, 
...
\end{align}
\end{subequations}
as the diagonal elements of the Pauli compoents of the 1 and 2RDMs. They may be thought of themselves
...
allows to drastic simplifications in the development of many--body theory as the hermeticity
