...
in an external potential $\op{v}^\mathrm{eff}$. Given an configuration subset, $\mathcal{K}^{N_\mathrm{el}}_I \subset \mathcal{C}$,
the energy of a non--interacting system is given by
\begin{equation}
\label{eq:NIE}
E_\mathrm{NI} = \sum_{i \in \mathcal{K}_I^{N_\mathrm{el}}}  \epsilon_i.
\end{equation}
If one wishes to minimize
$E_\mathrm{NI}$ over the possible Slater determinants which may be constructed from $\mathcal{C}$,
one must select the $N_\mathrm{el}$ orbitals with the lowest eigenenergy to build the determinant.
