...
constraint, we may construct a basis for $\hilb{H}^N_-$ with elements \cite{Ostlund12_book}
\begin{equation}
  \label{eq:SlaterDet}
...
Namely, if $\hilb{H}^1$ represents a non-relativistic spin--1/2 fermion, $\set{\ket{\vc{r},\sigma}}$ forms a complete basis for $\hilb{H}^1$ 
...
\begin{equation}
  \inner{\vc{x}'_1,\vc{x}'_2,\ldots,\vc{x}'_N}{\vc{x}_1,\vc{x}_2,\ldots,\vc{x}_N} = \prod_{i=1}^N \delta^4(\vc{x}'_i - \vc{x}_i),
\end{equation}
\begin{equation}
...
(see \cref{sec:NRH,sec:RELH} for instance), however in this section we will focus on the general presentation
...
\begin{align}
  \op{O}^1 = \op{1}_1 \op{O}^1 \op{1}_1 = \iint_{\mathbb F} 
    \ket{\vc{x}} \op{O}^1(\vc x; \vc x') \bra{\vc x'} \dd^4\vc{x} \dd^4\vc{x}',
\end{align}
where
\begin{align}
\op{O}^1(\vc x; \vc x') = \innerop{\vc{x}}{\op{O}^1}{\vc{x}'}.
\end{align}
For $\ket{\psi},\ket{\chi} \in \hilb{H}^1$ we have
\begin{align}
  \label{eq:SpinorOpInner1}
  \innerop{\psi}{\op{O}^1}{\chi} &= \iint_{\mathbb F}
    \psi^*(\vc{x}) \op{O}^1(\vc x; \vc x') \chi({\vc{x}'}) \dd^4\vc{x} \dd^4\vc{x}' \nonumber \\
  &= \int_{\mathbb R^3} 
    \begin{bmatrix}
      \psi^\alpha(\vc{r}) \\
      \psi^\beta(\vc{r})
...
\op{O}^1(\vc x; \vc x') = \delta^3(\vc{r} - \vc{r'}) \op{O}^{1,\sigma\sigma'}(\vc{r}').
\end{equation}
...
\begin{align}
  ...
  = \idotsint_{\mathbb F}
    ...
    &\qquad\ket{\vc{x}_1,\ldots,\vc{x}_N} \op O^N (\vc{x}_1,\ldots,\vc{x}_N; \vc{x}'_1,\ldots,\vc{x}'_N) \bra{\vc{x}'_1,\ldots,\vc{x}'_N} 
\end{align}
...
\begin{equation}
  ...
  \op O^N (\vc{x}_1,\ldots,\vc{x}_N; \vc{x}'_1,\ldots,\vc{x}'_N) = 
...
\begin{equation}
\label{eq:SpinorOpN}
\spop{O}^N (\vc{r}_1,\ldots,\vc{r}_N) \equiv \sum_{\sigma_1\cdots\sigma_N}\sum_{\sigma'_1\cdots\sigma'_N}  \op{O}^{N,\sigma_1\sigma_1'\cdots\sigma_N\sigma_N'}(\vc{r}_1,\ldots,\vc{r}_N) 
...
\end{align}
The product operation of operators denoted $\spop{O}^N(\vc{r}_1,\ldots,\vc{r}_N)$ and their action onto spinor wave functions 
...
For the remainder of this work, we will refer to the projections $\spop{O}^N(\vc{r}_1,\ldots,\vc{r}_N)$ as 
...
\begin{equation}
  ...
  \spop{O}^N (\vc{r}_1,\ldots,\vc{r}_N) = \sum_{K_1\cdots K_N}\op{O}^{N,K_1\cdots K_N}(\vc{r}_1,\ldots,\vc{r}_N) \otimes \bigotimes_{i = 1}^N \pauli{K_i}
\end{equation}
where 
\begin{equation}
\label{eq:PauliMats}
...
\begin{align}
  \label{eq:MOpSpace}
  &\spop{O}^M(\vc{r}_i,\vc{r}_j,\ldots) \Phi_I^N(\vc{x}_1,\vc{x}_2,\ldots,\vc{x}_N) = \nonumber \\ &\qquad \quad
    \frac{1}{\sqrt{N!}} \sum_{\xi \in S_N(\hilb{K}^N_I)} \Sign{\xi} \text{ } 
    \left(\spop{O}^M(\vc{r}_i,\vc{r}_j,\ldots) \phi_{\xi(i)}(\vc{x}_i)\phi_{\xi(j)}(\vc{x}_j)\cdots\right)
...
&\innerop{\Psi^N}{\sum_i \op{O}^1(i)}{\Psi^N} = \sum_{\sigma_1\sigma_1'} \int_{\mathbb{R}^3} 
  \left.\left( \op{O}^{1,\sigma_1\sigma_1'}(\vc{r}_1) \gamma^1(\vc{r}_1,\sigma'_1; \vc{r}'_1, \sigma_1) \right) \right \vert_{\vc{r}'_1 = \vc{r}_1}
    \dd^3\vc{r}_1,\\
...
&\qquad \left.\left( 
    \op{O}^{2,\sigma_1\sigma_1'\sigma_2\sigma_2'}(\vc{r}_1,\vc{r}_2) \gamma^2(\vc{r}_1,\sigma'_1,\vc{r}_2,\sigma'_2; \vc{r}'_1, \sigma_1,\vc{r}'_2, \sigma_2)
...
\begin{equation}
  ...
  \spn{\gamma}^P(\vc{r}_1,\vc{r}_2,\ldots;\vc{r}'_1,\vc{r}'_2,\ldots ) \equiv 
...
\end{equation}
This realization allows for a simplification of  \cref{eq:GenDenMatInner}, such that (see \cref{apx:SpinorOp})
\begin{subequations}
...
\end{align}
\end{subequations}
The expressions for the 2RDM may be derived from the Grassman product form in \cref{eq:RDMSD}. It is important to note that
...
\begin{align}
  \label{eq:Densities}
&\spn{\rho^1}(\vc{r}) = \sum_K \rho^{1,K}(\vc{r}) \otimes \pauli{K}\\
&\spn{\rho^2}(\vc{r},\vc{r}') = \sum_{KK'} \rho^{2,KK'}(\vc{r},\vc{r}') \otimes \pauli{K}\otimes \pauli{K'}
\end{align} 
...
$\tau$ operators generates all $M$--particle Slater determinants from the $N$--particle reference and is complete given any reference \cite{Dalgaard79_169}.
...
%  %% \label{eq:FieldOpRep}
%  %% \begin{align}
%  %%   &\op{O}_\hilb{F}^1 = \int_{\mathbb F} \psi^\dagger(\vc{x}_1) \,\spop{O}^1(\vc{r}_1) \,\psi(\vc{x}_1) \,\,\dd^4 \vc{x}_1, \\
%  %%   &\op{O}_\hilb{F}^2 = \iint_{\mathbb F} \psi^\dagger(\vc{x}_2)\psi^\dagger(\vc{x}_1) \,\spop{O}^2(\vc{r}_1,\vc{r}_2) \,\psi(\vc{x}_1)\psi(\vc{x}_2) 
...
%  %%     \left.\left(\op{O}^{1,\sigma_1\sigma'_1} (\vc{r}_1) \, \psi^\dagger(\vc{r}'_1,\sigma_1) \psi(\vc{r}_1,\sigma'_1) \right) \right\vert_{\vc{r}'_1 = \vc{r}_1 } 
%  %%     \,\dd^3 \vc{r}_1. 
%  %% \end{align}
...
%  %%     \left(\op{O}^{1} (\vc x_1'; \vc{x}_1) \, \op{\gamma}^1(\vc{x}_1; \vc{x}'_1) \right)     \,\dd^4 \vc{x}_1\,\dd^4 \vc{x}'_1, \qquad \op{\gamma}^1(\vc{x};\vc{x}') = \psi^\dagger(\vc{x}') \psi(\vc{x}).
%  %% \end{align}
...
%  %% \gamma^1(\vc{x};\vc{x}') = \innerop{\Psi}{\op{\gamma}^1(\vc{x};\vc{x}')}{\Psi}_\hilb{F},
%  %% \end{equation}
...
of \cref{eq:QWave} with this Hamiltonian as \cite{Napolitano17_book} 
\begin{equation}
...
\end{equation}
which admits a sole minimum at the ground electronic state due to the variational theorem \cite{Ostlund12_book,Napolitano17_book}.
...
minimizing Slater determinant of \cref{eq:EnergyFunctional} is known as the Hartree--Fock (HF) approximation \cite{Ostlund12_book,Yang89_book}.
...
\begin{align}
  &H^\core[\gamma^1] = 
    \iint_\mathbb{F} \left( \op{h}(\vc{x}_1; \vc{x}_1') + \op{v}(\vc{x}_1; \vc{x}_1')\right) \gamma^1(\vc{x}'_1; \vc{x}_1) \, \dd^4\vc{x}_1 \dd^4\vc{x}'_1\,\,  \label{eq:CoreFunc}
    \\
  &J[\gamma^1] = \iiiint_\mathbb{F} \op{g}(\vc{x}_1, \vc{x}_2; \vc{x}_1', \vc{x}_2') 
  ...
  &K[\gamma^1] = \iiiint_\mathbb{F} \op{g}(\vc{x}_1, \vc{x}_2; \vc{x}_1', \vc{x}_2') 
...
\begin{align}
  ...
  \int_{\mathbb F} \left( \op{F}^\HF(\vc{x}_1; \vc{x}_1')\delta_{ij}   - \delta^4(\vc{x} _1- \vc{x}_1')\lambda_{ij} \right)
...
&\op{F}^\HF(\vc{x}; \vc{x}') = 
  \frac{\delta E^\mathrm{HF}[\gamma^1]}{\delta \gamma^{1}(\vc{x}'; \vc{x})} = \op{H}(\vc{x};\vc{x}') + \op{J}(\vc{x};\vc{x}') - \op{K}(\vc{x};\vc{x}'),\\
&\op{H}^\core(\vc{x}; \vc{x}') = \frac{\delta H[\gamma^1]}{\delta \gamma^1(\vc{x}'; \vc{x})},\qquad 
\op{J}(\vc{x}; \vc{x}') = \frac{\delta J[\gamma^1]}{\delta \gamma^1(\vc{x}'; \vc{x})}, \qquad
\op{K}(\vc{x}; \vc{x}') = \frac{\delta K[\gamma^1]}{\delta \gamma^1(\vc{x}'; \vc{x})} . \label{eq:FockComponents}
\end{align}
\Cref{eq:HFNonCan} is the general form of a set of non--linear integrodifferential equations known as the Hartree--Fock equations and $\op{F}^\HF$ is
known as the Fock operator corresponding to \cref{eq:GenElHam} \cite{Ostlund12_book,Yang89_book}. The non--linearity of \cref{eq:HFNonCan} comes in the fact that the functionals
...
Using the fact that $\gamma^1$ (and thus $\op F^\HF$) is invariant to unitary transformation of the orbitals which comprise its construction (\cref{eq:UnitarySD}),
we may choose to transform \cref{eq:HFNonCan} in such a way as to diagonalize the matrix of Lagrange multipliers, 
$\vc U: \lambda_{ij} \rightarrow \tilde\lambda_{i} \delta_{ij}$,
\begin{equation}
\label{eq:HFEq}
\int_{\mathbb F} \op{F}^\HF(\vc{x}_1; \vc{x}_1') \tilde\phi_i(\vc{x}_1'), \dd^4\vc{x}'_1 = \tilde\lambda_i \tilde\phi_i(\vc{x}_1), 
  \qquad \tilde\phi_i = \sum_j U_{ij}\phi_j. 
\end{equation}
...
Due to \cref{eq:UnitarySD}, the Slater determinant which is formed from $\{\tilde \phi_i\}$ is equivalent to the one formed
...
\end{equation}
$\{ \epsilon^\HF_i \}$ will be referred to as the set of canonical HF eigenenergies.
...
realization of Roothaan \cite{Roothaan51_RMP} that \cref{eq:HFEq} may instead be cast into a numerical linear algebra problem by expanding the
...
\begin{equation}
\label{eq:OccRHEq}
\spvc F^\HF \,\spvc C^\HF_o = \spvc S\, \spvc C^\HF_o\, \vc \epsilon_o^\HF
\end{equation}
...
\begin{subequations}
\begin{align}
  &\spvc{F}^\HF=  \spvc{H}^\core + \spvc J - \spvc K, \label{eq:HFMat}\\ 
...
of second quantization) in the basis set. $\vc \epsilon_o^\HF \in
...
\begin{equation}
\label{eq:RHEq}
\spvc F^\HF \,\spvc C^\HF = \spvc S\, \spvc C^\HF\, \vc \epsilon^\HF,
\end{equation}
where $\spvc{C}^\HF \in \MatSpace{C}{2N_b}{2N_b}, \vc \epsilon^\HF \in \MatSpace{R}{2N_b}{2N_b}$ represent a full rank 
...
obtain the full expansion of \cref{eq:ExcitationOp}; a feat which is computationally intractable for all but
...
wave function \cite{Kohn64_864}. More generally, the HK theorem may be
extended to the 1RDM if the external potential has spin structure or is
generally non--local in character \cite{Gilbert75_2111}. Thus the total energy of
...
reproduce certain physical quantities, the cost--to--accuracy ratio 
...
functional, such as the Thomas--Fermi functional \cite{Yang89_book}, for practical calculations.
...
wave function to represent the true many--body wave function \cite{Sham65_1133}. As
...
\begin{equation}
  \label{eq:KSEqCan}
  \int_{\mathbb F} \op{F}^\KS(\vc{x}_1; \vc{x}_1') \phi_i(\vc{x}_1') \,\dd^4\vc{x}'_1 = 
...
\begin{align}
  ...
  &\op{F}^\KS(\vc{x}; \vc{x}') = 
    \op{H}^\mathrm{core}(\vc{x}; \vc{x}') + \op{J}(\vc{x}; \vc{x}') +
    \op{V}^{xc}(\vc{x}; \vc{x}') \\
  &\op{V}^{xc}(\vc{x}; \vc{x}') = \frac{\delta \tilde E^{xc}[\gamma^1_s]}{\delta \gamma^1_s(\vc{x}';\vc{x})},
\end{align}
and $\op V^{xc}$ is referred to as the \emph{exchange--correlation} (xc) potential. \Cref{eq:KSEqCan}
...
\cite{HeadGordon05_4009,Casida95_book}.
...
the Roothaan--Hall equation of \cref{eq:RHEq} \cite{Johnson92_557},
\begin{equation}
\label{eq:RH-KSEq}
\spvc F^\KS \,\spvc C^\KS = \spvc S\, \vc C^\KS\, \vc \epsilon^\KS,
\end{equation}
where
\begin{align}
...
\end{align}
Here, $\spvc{C}^\KS \in \MatSpace{C}{2N_b}{2N_b}$ and $\vc\epsilon^{KS} \in \MatSpace{R}{2N_b}{2N_b}$ are the KS-MO
...
\end{align}
\end{subequations}
Somewhat erroneously, the matrix $\spvc P$ is referred to as the one particle density matrix (1PDM), which is not to be
confused with the 1RDM, although they are related through the above expansion. 
%%
%
%We may further define an analogus two
...
%\end{equation}
The utility of the 
%definions of the 1PDM and 2PDM  are 
definition of the 1PDM is
clearly seen in the evaluation of expectation values
...
from the electronic Hamiltonian \cite{Li15_154109,Paldus03_67}. In general, if the electronic Hamiltonian contains non--trivial 
spin structure,
...
\begin{equation}
  ...
  \end{bmatrix}.
\end{equation}
\Cref{eq:USCFEq} is referred to as the Pople--Nebst equation \cite{Ostlund12_book}, and the restriction on the SCF
...
may be energetically ordered in and of themselves, but no global orbital energetic ordering is implied.
...
\begin{equation}
  \left[ \spvc F^\mathrm{GSCF}, \spvc S_z \right] = 
...
\begin{equation}
  \label{eq:USCFFock}
  \spvc F^\mathrm{USCF} = \vc F^0 \otimes \pauli{0} + \vc F^3 \otimes \pauli{3}
                      = 
                      \begin{bmatrix}
                        \vc{F}^0 + \vc F^3 & \vc{0} \\ \vc{0} & \vc{F}^0 - \vc F^3
                      \end{bmatrix} \quad \text{s.t.} \quad \left[ \spvc F^\mathrm{USCF}, \spvc S_z \right] = \vc 0,
\end{equation}
...
\end{equation}
where we have used the fact that $\spvc\, C^\dagger \spvc\, S \spvc C = \vc I$ by convention. Here $N^\alpha$ and 
...
spin configuration by restricting the number of $\alpha$ and $\beta$ orbitals in the resulting determinant.
The ground state configuration is constructed by choosing the $N^\alpha$ orbitals from $\mathcal C^\alpha$ with
...
This restricting does not, however, guarantee a particular spin--multiplicity as the resulting wave function 
is not guaranteed to be an eigenfunction of $\op S^2$. 
USCF is typically used in cases where the electronic Hamiltonian is either spin independent and the desired 
determinant is restricted to have $N^\alpha \neq N^\beta$ (i.e. open--shell systems), or when the electronic Hamiltonian depends only on
...
\end{equation}
This block structure arises from that fact that $\op S^2$ symmetry implies simultaneous
...
\begin{equation}
\spvc{F}^\mathrm{RSCF} = \vc F^0 \otimes \pauli{0},
\end{equation}
...
orbital of a particular spin set is the orbital in the other spin set which has the same spatial part and energy eigenvalue
...
and the structure of the 1PDM which arises from their solution. For a general Fock matrix $\spvc F$ and corresponding
...
total energy of the system.  In the absence of external fields, $\op{H}^\NR$ takes the form \cite{Ostlund12_book,Napolitano17_book,Tully98_407}
\begin{equation}
  \label{eq:FullHNR}
...
\begin{align}
  ...
  &\spop{\vc{p}}(\vc{r}_\xi) = -\ii \nabla_\xi \otimes \op{I}_S(\xi) \quad \Longrightarrow \quad 
  \spop{T}^\NR(\vc{r}_\xi) = - \frac{1}{2m_\xi} \Delta_\xi \otimes \op{I}_S(\xi),
\end{align}
\begin{align}
  \label{eq:SpinorCoulomb}
  &\spop{g}^C(\vc{r}_\xi,\vc{r}_\zeta) = 
...
\emph{not} spin--1/2 fermions and thus carry a much more complicated spin structure \cite{Levine75_book}. For electrons, $\op{I}_S=\pauli{0}$.
...
of the original Schr\"{o}dinger Hamiltonian of the general form \cite{Napolitano17_book}
\begin{equation}
\label{eq:SchHam}
...
\end{equation}
This assumption is referred to as the Born--Oppenheimer approximation \cite{Tully98_407} (a concept distinct from from the 
...
component of the wave function and is valid in the inertial frame of the electrons \cite{Tully98_407},
\begin{equation}
\label{eq:NRBOH}
...
\begin{equation}
  \label{eq:GauNuc}
\rho^{1,0}_{nuc,A}(\vc{R}) = \left(\frac{\upsilon_A}{\pi}\right)^{3/2}e^{-\upsilon_A(\vc{R} - \vc{R}_A)^2},
\end{equation}
where $\vc{R}_A$ is the classical nuclear position and 
\begin{equation}
\upsilon_A = \frac{3}{2 \zeta}, \qquad \zeta = 0.836 M_A^{1/3} + 0.570.
\end{equation}
...
Fock matrix components of \cref{eq:FockComponents} take the form \cite{Yang89_book},
\begin{subequations}
...
\begin{align}
  ...
  &\op K(\vc x; \vc x') = \frac{\gamma^1(\vc x; \vc x')}{\vert \vc x - \vc x' \vert}
\end{align}
\end{subequations}
...
matrices need not exhibit the same spin structure as the Hamiltoninan which they represent. This phenomena
is known an symmetry breaking, and is a well studied problem in electronic structure theory \cite{Li15_154109,Paldus03_67}.
The simplest example of this is the treatment of open--shell systems via \cref{eq:NRBOH}: the treatment of
unpaired spins necessarily introduces an explicit treatment of spin through $\vc F^3 / \vc P^3$ despite the
...
of the electronic spin degrees of freedom in the strong field limit \cite{Levine75_book}, whereas in the weak field limit, intrinsic magnetic
...
of classical mechanics, such a simplistic assumption yields qualitatively incorrect model physics even for light elements such as Carbon and Oxygen \cite{Pyykko12_45}.
...
Dirac equation \cite{Wolf15_book,Faegri07_book},
\begin{equation}
\label{eq:DiracEq}
...
the quantum mechanical behavior of a spin--1/2 fermion in full adherence to the principles of special
relativity, i.e. it is fully covariant under Lorentz transformations \cite{Wolf15_book,Faegri07_book}.
...
are often referred to as bispinors \cite{Wolf15_book} as they are constructed from four complex components (hence 
...
the subject for a more thorough treatment of these matters \cite{Wolf15_book,Faegri07_book}. There are, however, several
...
Unlike the single particle Schr\"{o}dinger equation, the spectrum of \cref{eq:DiracHam} is partitioned
into three regions relative to the rest mass of the electron: a strictly positive energy continuum 
$E > 0$, a strictly negative energy continuum $E < -2c^2$, and a set of discrete states with $E \in (-c^2,0)$ which
...
equation \cite{Feynman49_749}. As this work is to treat the electronic problem in molecular physics, we will
...
crucial to a rigorous treatment of relativistic effects in molecular physics.
...
\cref{eq:FockEq}. This is due to the fact that in our presentation of mean--field quantum mechanics,
the premise was built on \emph{minimization} of the energy functional in \cref{eq:EnergyFunctional}
over Slater determinants. The analogous problem of determining a mean--field electronic solution
to the Dirac equation is a constrained minimization over only \emph{electronic} solutions, 
as there are an infinite number of continuum positronic solutions. Minimization of 
\cref{eq:EnergyFunctional} over purely electronic Slater determinants in generally impractical using standard 
optimization techniques and requires rather opaque projection schemes to avoid variational
collapse into the positronic regime \cite{Wolf15_book}. However, as we know \emph{a priori} that
...
\subsection{The Dirac--Coulomb Molecular Hamiltonian}
\label{sec:DCBHam}
...
the context of practical calculations on molecular systems \cite{Liu14_983,Liu15_631}. Here, we will work with
\emph{approximate} relativistic descriptions of molecular quantum mechanics within a Hamiltonian
formulation, which have been shown to have good agreement with experiment \cite{Wolf15_book,Faegri07_book,Visscher03_book,Liu05_CP}.
...
theory \cite{Wolf15_book}. Further, while no approximation of the quantum nature of the nuclei was required in the 
...
magnetic interactions between electrons due to the disparity in mass between electrons and nuclei \cite{Wolf15_book}.
This approximation will drastically simplify the resulting quantum mechanical treatment.

Within the context of the electronic problem, relativistic quantum mechanics is approximately governed
by the Dirac--Coulomb equation in cases where retardation effects may be considered negligible,
\begin{equation}
\label{eq:DCEq}
\op{H}^\DC \ket{\Psi_{el} (t)} = \ii \partial_t \ket{\Psi_{el} (t)},
\end{equation}
where $\op{H}^\DC : \hilb{H}^\REL \rightarrow \hilb{H}^\REL$ is the Dirac--Coulomb (DC) Hamiltonian.
In the absence of external fields , the DC Hamiltonian for $N$ electrons is given in atomic units by \cite{Wolf15_book,Faegri07_book}
\begin{equation}
  \label{eq:DCHam}
...
\cref{eq:SpinorCoulomb}, respectively.
%\begin{align}
...
non--relativistic spinor wave functions, many--body bispinor wave functions may be written as the anti-symmetric
tensor product of single particle bispinors which span $\hilb H^\FrC$ (i.e. in the sense of
\cref{eq:SlaterDet,eq:SDBasis}).

Due to the increased dimensionality of the wave functions in \cref{eq:DiracEq,eq:DCEq}, the RDM must now represent 
...
over the dimension added as a result of the bispinor nature of the wave function. However, as we will be solely
